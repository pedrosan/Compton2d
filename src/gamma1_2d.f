...
c     gmin(j,k) = dmin1(.9*gmax(j,k), 1.0d1)  ! Xuhui Chen 11/2/08

  10  gmin(j,k) = gmin(j,k)/1.01
c      if (gmin(j,k).le.1.01) goto 15 ! Xuhui 05/25/09
...
c      if (fth.lt.fnth) goto 10 ! Xuhui 05/25/09
  ...
  15  continue
     ...
     1                 *(gmax(j,k)**p_2 - gmin(j,k)**p_2)/p_2
      endif
