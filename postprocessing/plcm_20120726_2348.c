      ...
      for (l=0; l<n_bins; l++) {
        if( E >= E0[l] && E < E1[l] ) {
          ...
          if (k < st_max && l < n_bins && n < n_mu) {
            ...
            F2[k][n][l]+=(ew*ew);
            particle_sum[k][n][l]++;  
            if( l == 6 ) {  // GF: too ad-hoc to check the 7th array...
              //fprintf(fpcheck,"%e %e %e %e %e\n", t_bound, time, E, ew, mu);
              fprintf(fpcheck," %e %e %e %e %f  %4d %2d %2d  %f %f\n", t_bound, time, E, ew, mu, k,n,l,mu0[n],mu1[n]);
            }
          }
        }
      }
...
for (n=0; n<n_mu; n++) {
   ...
   for (l=0; l<n_bins; l++) {
      fprintf (fpout, "#energy range%i: %e %e (keV)\n", l, E0[l], E1[l]);
   }
   ...
   for (k=0; k<st_max; k++) {
      ...
      for (l=0; l<n_bins; l++) fprintf (fpout, " %e", F1[k][n][l]);  // GF
      for (l=0; l<n_bins; l++) fprintf (fpout, " %e", F2[k][n][l]);  // GF
      for (l=0; l<n_bins; l++) fprintf (fpout, " %e", sF[k][n][l]);  // GF
