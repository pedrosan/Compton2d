...
c
     ...
     1    'What?! wmu too extreme! Eta=',wmu
      if (wmu.gt.1.d0) wmu = 1.d0
      if (wmu.lt.-1.d0) wmu = -1.d0
c       if (wmu.gt.9.9999999d-1) wmu = 9.9999999d-1
c       if (wmu.lt.-9.9999999d-1) wmu = -9.9999999d-1
c          
...
c         endif
c       
c       if (Eta.gt.9.9999999d-1) Eta = 9.9999999d-1
c       if (Eta.lt.-9.9999999d-1) Eta = -9.9999999d-1
c
       disp = Eta*rpre
...
c      else      
c         Eta = (trld + Eta*rpre)/rnew ! 17/10/2012 Xuhui
          Eta = (f + Eta*rpre)/rnew

c      endif
      if (abs(Eta).gt.1.d0+1.d-15)write(*,'(e24.17)')
     1    'What?! Eta too extreme! Eta=',Eta
...
c         if (idead .eq. 4) then
c            kph = 0
c            go to 100
c         endif
c
          go to 110
c          go to 100 ! Xuhui 3/4/09
        else
c     
          kph = knew
          jph = jnew
