...
c       integral variables
       ...
       read(nunit_misc,'(i10)')T_const
       do i=1,30
         read(nunit_misc,'(i10)')pfrl(i)
       enddo
       ...
       read(nunit_misc,'(2i10)')cr_sent, upper_sent
       ...
       do j=1,nz
         ...
         enddo
       enddo
       read(nunit_misc,'(i10)')dh_sentinel
       read(nunit_misc,'(i10)')star_switch
       read(nunit_misc,'(i10)')spec_switch
       read(nunit_misc,'(4i10)')inj_switch,inj_dis,g2var_switch,pick_sw
       ...
       read(nunit_misc,'(4i10)')split1, split2, split3, spl3_trg
c
c       double precision variables
      read(nunit_misc,'(7e14.7)') r_flare, z_flare, t_flare, sigma_r, 
     ...
     2                 gbar_nth(j,k), N_nth(j,k)
      ...
      do j=1,nz
        do k=1,nr
        read(nunit_misc,'(2e14.7)')q_turb(j,k),turb_lev(j,k)
        enddo
      enddo
     ...
     1                             f_ref, A_in, n_disk
      ...
      read(nunit_misc,'(10e14.7)') inj_g1, inj_g2, inj_p, inj_t,inj_gg,
     1                 inj_sigma, inj_L, inj_v, g_bulk, pick_rate
      read(nunit_misc,'(2e14.7)') r_esc, r_acc
