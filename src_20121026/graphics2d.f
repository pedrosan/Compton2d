...
c
clc	This routine is called at the end of the problem by routine XEC
clc	It prepares and prints information that may be read and displayed
clc	by GNUPLOT (a graphing and plotting program)
c
c
      integer nunit_sp, nunit_ph, nunit_lc, lgp,
     1        n_format, n, rl, r1, r2, nunit_temp,
     2        n5_sentinel, n5, nt2, n10, n100, n1000
      integer j, k
c
...
c
      character *30 fname
      character *30 form(kmax), t_profile
c
...
c
c      data t_profile / 'temp_profile0001.dat' /
      data t_profile / 'temp/profile0001.dat' / ! Xuhui
c
      nunit_sp = 12
      nunit_ph = 13
      nunit_lc = 14
...
c
c         write(*,*) 'hu = ',hu(lgp)
c         write(*, fmt=form(nmu)) hu(lgp), (foutsp(n), n=1,nmu)
c
...
c
c            write(*,*) 'lgp = ',lgp
c            write(*,*) 'lc_flux = ',foutsp(lgp)
c
 20      continue
c
         if (ncycle.eq.1) then
c            open(unit=nunit_lc, file=lcname(n), access='direct', 
c     1           form='formatted', recl=rl, status='unknown')
            open(unit=nunit_lc,file=lcname(n)) !Xuhui Chen 6/27/08
         else
c            open(unit=nunit_lc, file=lcname(n), access='direct', 
c     1           form='formatted', recl=rl, status='old') 
            open(unit=nunit_lc,file=lcname(n),access='append') !Xuhui Chen 6/27/08

         endif
c         write(nunit_lc, fmt=form(nph_lc), rec=r1) time0, (foutsp(lgp), 
c     1                                 lgp=1,nph_lc)
c         write(nunit_lc, fmt=form(nph_lc), rec=r2) (time + dt(1)), 
c     1                              (foutsp(lgp), lgp=1,nph_lc)
...
c
  ...
  50  continue
      T_av = sum_NT/sum_N
c
c      write(nunit_temp, 60) time0, T_av
c      write(nunit_temp, 60) (time + dt(1)), T_av
