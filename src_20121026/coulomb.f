...
c
c
c     This subroutine initializes the heating rates and 
c     dispersion coefficients for Coulomb scattering off of 
c     protons and electrons (i.e., Moeller scattering).  These
c     quantities are read in from files and stored in the 
c     arrays lib_dg_ce, lib_disp_ce, lib_dg_cp, and lib_disp_cp.
c     The gamma-1 grid from the files is stored in rate_gm1.
c                        JDF Sept. 2004
      subroutine Coulomb
      implicit none
      include 'general.pa'
...
c
c     Find the min and max electron and proton temperatures 
c     of all the zones.  Let tea go from 50 below to 50 above
c     the min and max tea.
      tea_min=1.e10
      tea_max=0.
      tna_min=1.e10
      tna_max=0
...
c
c      write(*,*) 'tea_min:  ', tea_min
c      write(*,*) 'tea_max:  ', tea_max
c      write(*,*) 'tna_min:  ', tna_min
c      write(*,*) 'tna_max:  ', tna_max
c
c     Loop over all electron temperatures
...
c         write(4,*) 'kTe:  ', kTe
c         write(4,*) 'name_dge:  ', name_dge
         inquire(file=name_dge, EXIST=ex)
        if(ex) then
           call read_rate_file(lib_dg_ce, lib_disp_ce, name_dge, i)
        else
c          The marker for not having rates for a certain
c          temperature is the first rate is equal to zero.
           lib_dg_ce(1,i)=0.0
           lib_disp_ce(1,i)=0.0
        endif
c        write(4,*) 'lib_dg_ce(1,i),i:  ', lib_dg_ce(1,i), i
 20   continue
c
c
c     This if round tna_min to the nearest 10 KeV.
      ...
      if(num_steps.gt.num_temp_max) num_steps=num_temp_max
c      num_steps=5
c     loop over the proton temperatures in all zones
      do 22 j=1, nz
         do 21 k=1, nr
            i = k+(j-1)*nr
            if( i.gt.num_steps ) goto 23
c           This rounds tna to the nearest 10 KeV and saves
c           it as kTp
...
c            write(4,*) 'kTp:  ', kTp
c            write(4,*) 'tna:  ', tna(j,k)
c            write(4,*) 'name_dgp:  ', name_dgp
c            write(4,*) 'jk code number:  ', k+(j-1)*nr
            inquire(file=name_dgp, EXIST=ex)
            if(ex) then
               call read_rate_file(lib_dg_cp, lib_disp_cp, name_dgp, i)
            else
c              The marker for not having rates for a certain
c              temperature is the rate is equal to zero.
               lib_dg_cp(1,i)=0.0
               lib_disp_cp(1,i)=0.0
            endif
 21      continue
...
c
c
c     This subroutine gets the name of a rate file,
c     i.e., a dgeXXXX.dat or dgpXXXX.dat file.
...
c
c
c     This subroutine reads in values for dg and disp
c     (which represent the heating/cooling rate and 
c     dispersion coefficient, respectively)
c     from a rate file (dgeXXXX.dat or dgpXXXX.dat file.
...
c
      integer i, m
c     i is the index for temperature, m is the index
c     for energy bin
      double precision g_read, dg(num_nt, num_temp_max), 
     1                 disp(num_nt, num_temp_max)
      character *30 name
c     format for reading from rate file
